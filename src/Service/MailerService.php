...
class MailerService {
     ...
     * @param object $formData    object contenant les donées soumise dans le formulaire
     */
    public function sendNotificationReservation($locataire)
    {             
      //envoi du mail  
      $message = (new \Swift_Message('Tarbes Appart'))
              ->setFrom('florent.bvs@gmail.com')
      ...
      if($locataire->getStatut() == 1){ //résa validé
        $message->setBody($this->renderer->render('layout_emails/resaValid.html.twig', ['img_bg1'  => $img_bg1,'locataire' => $locataire]), 'text/html');
      }else{
        $message->setBody($this->renderer->render('layout_emails/resaAnnule.html.twig', ['img_bg1'  => $img_bg1,'locataire' => $locataire]), 'text/html');
      }
